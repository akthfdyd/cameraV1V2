...
object CameraHelper {
     ...
     */
    fun getOutputMediaFile(path: File, type: Int): File? {
...
//                Environment.DIRECTORY_PICTURES), "CameraSample")
        val mediaStorageDir = File(path, "Camera")
        ...
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.d("cameraV1V2", "Camera failed to create directory")
                return null
            }
        }
        ...
        if (type == MEDIA_TYPE_IMAGE) {
            ...
            Log.d(
                "cameraV1V2", "Camera path = " + mediaStorageDir.path + File.separator +
                        "VID_" + timeStamp + ".mp4"
            )
            mediaFile = File(
                mediaStorageDir.path + File.separator +
